...
const login = async (req) => {
  ...
  try {
    ...
    let sql = "SELECT cd.car_regis,cd.id FROM car_detail cd INNER JOIN login lo on cd.car_regis = lo.car_regis where lo.car_regis = ? AND lo.otp_pin = ?";
    const [rows] = await db.execute(sql, params);
    const id = rows.length < 1 ? "0" : rows[0]["id"]
    const have = rows.length < 1 ? 0 : 1 
    
    //console.log("login model: " + JSON.stringify(rows));
    rtValue = { folder_name: id  ,have : have};
